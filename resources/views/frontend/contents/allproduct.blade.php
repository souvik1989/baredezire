...
@section('title', $category->meta_title ?? "All Products")
...
<section class="inner-banner">
    ...
    @if ($category)
                    <li><a href="{{route('maincategory-product',[$category->slug])}}">{{$category->name}}</a></li>
                  @endif
                </ul>
            </div>
...
<section class="filter-wrap d-none d-lg-block d-md-none d-sm-none">
    <div class="container-fluid">
        <div class="row">
           <div class="col-md-8">
               <div class="sorting-sec" id="sorting">
                   <p> Sort By : </p>
                 <span class="sorting-item">
                 <a href="{{route('main.sorting',[$category->slug,'low-to-high'])}}" class="more">Low to high</a>
                 </span>
                    <span class="sorting-item">
                    <a href="{{route('main.sorting',[$category->slug,'high-to-low'])}}" class="more">High to low</a>
                 </span>
                 <span class="sorting-item">
                    <a href="{{route('main.sorting',[$category->slug,'new-arrivals'])}}" class="more">New arrivals</a>
                 </span>
                 <span class="sorting-item">
                    <a href="{{route('main.sorting',[$category->slug,'most-viewed'])}}" class="more">Most viewed</a>
                 </span>
                 <span class="sorting-item">
...
<div class="modal fade filter-modal" id="exampleModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
        <form id="filterForm" action="{{ route('filter.fetchProducts') }}" method="get">
...
<script>
    function clearAllFilters() {
        window.location.href = "{{ route('maincategory-product',$category->slug) }}";
    }
</script>
<script>
...
document.getElementById('filterForm').addEventListener('submit', function(event) {
    ...
    var url = "{{ route('filter.fetchProducts') }}?";
