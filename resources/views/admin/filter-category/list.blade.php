...
@section('title', 'Filter Category List')
...
<div class="content-wrapper">
  ...
  <div>
    <a href="{{ route('filter-category.create') }}" class="btn btn-primary font-weight-bold mb-3">+ Add New Category</a>
  </div>
  ...
  <div class="row">
    <div class="col-lg-12 grid-margin stretch-card">
      <div class="card">
        <div class="card-body">
          <h3 class="card-title">Filter Categories</h3>
          <hr>
  ...
  <td>
                        <a href="{{ route('filter-category.edit', $category) }}" class="btn btn-info btn-sm">
                          <i class='bx bx-edit-alt' ></i> 
                        </a>

                        <div class="d-inline-block">
                           <form action="{{route('filter-category.destroy', $category->id)}}"
                              method="POST"
                              >
                              @csrf
                              @method('DELETE')
                         ...
                         <div class="dropdown action-label">
                          <a class="btn @if(isset($category->status) && ($category->status=='1')) btn-primary @else btn-danger @endif  dropdown-toggle btn-sm text-white" data-bs-toggle="dropdown" aria-expanded="false">
                            ...
                            <div class="dropdown-menu">
                              <form action="{{route('filter.category.status',$category->id)}}"
                                  method="POST"
                                  >
                                  @csrf
                                  @method('PUT')
                 ...
                 @empty
                  <td colspan="7" class="text-center">Nothing is Listed Yet</td>
                @endforelse
